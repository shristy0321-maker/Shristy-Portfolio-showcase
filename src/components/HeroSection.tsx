...
const HeroSection = () => {
  ...
  const imageOpacity = useTransform(scrollYProgress, [0, 0.7], [1, 0.72]);
  ...
  return (
    <section ref={sectionRef} id="home" className="relative overflow-hidden pt-24 md:pt-28">
      <div className="section-container py-12 md:py-20">
        <div className="grid items-end gap-10 md:grid-cols-[1.05fr_0.95fr] md:gap-14">
          <div className="order-2 md:order-1 relative z-10">
            <div className="editorial-rule mb-8" />
            <p className="eyebrow mb-6">Product Management Portfolio</p>

            <h1 className="max-w-3xl text-[2.7rem] leading-[0.94] text-foreground sm:text-[3.5rem] md:text-[4.7rem] lg:text-[5.45rem]">
          ...
          <div className="order-1 md:order-2 relative min-h-[26rem] md:min-h-[40rem] flex items-end justify-center md:justify-end">
            <div className="pointer-events-none absolute inset-x-0 top-[18%] z-0 overflow-hidden text-center md:text-right">
              <span className="block text-[5rem] font-semibold uppercase tracking-[0.18em] text-muted sm:text-[6.5rem] md:text-[9rem] lg:text-[11rem]">
                SHRISTY
              </span>
            </div>

            <motion.div style={{ opacity: imageOpacity, y: imageY }} className="relative z-10 w-full max-w-[28rem] md:max-w-[32rem]">
              <div className="absolute inset-x-[12%] bottom-0 h-24 bg-accent/10 blur-3xl" aria-hidden="true" />
              <img
                src={profileImg}
                alt="Shristy Kumari portrait"
                className="h-[30rem] w-full rounded-lg object-cover object-center md:h-[40rem]"
              />
            </motion.div>
          </div>
        </div>
